...
CREATE TABLE IF NOT EXISTS `loa_evaluation_tool`.`TB_ACE_SUB_SCENARIOS` (
  ...
  `ASSEM_COST_PER_PIECES` DOUBLE NULL,
  `ASSEM_COST_TOTAL` DOUBLE NULL,
  `CREATE_DATE` DATETIME NOT NULL,
  `UPDATE_DATE` DATETIME NOT NULL,
  PRIMARY KEY (`PK_TB_ID`))
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `loa_evaluation_tool`.`TB_ACE_SCENARIOS` (
  ...
  `WEIGHTED_PHYSICAL_LOA` INT NULL,
  `WEIGHTED_COGNITIVE_LOA` INT NULL,
  ...
  `TOT_WEIGHTED_PHYSICAL_LOA` INT NULL,
  `TOT_WEIGHTED_COGNITIVE_LOA` INT NULL,
  `PROD_UNITS_PER_YEARS` INT(11) NULL,
