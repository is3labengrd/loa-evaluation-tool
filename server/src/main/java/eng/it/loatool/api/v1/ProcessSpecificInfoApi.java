...
import eng.it.loatool.process_specific_info.ProcessSpecificInfoService;
...
public class ProcessSpecificInfoApi {
	...
	public ResponseEntity<?> getAllProcessSpecificInfo() {
		return ResponseEntityTransformer.transformOk(generalCRUDService.getAll(processSpecificInfoRepository));
	}

	@GetMapping("/v1/process-specific-info/{id}")
	public ResponseEntity<?> getProcessSpecificInfo(@PathVariable("id") Integer id) {
		return ResponseEntityTransformer.transform(generalCRUDService.getOne(processSpecificInfoRepository, id));
	}
	...
	public ResponseEntity<?> updateProcessSpecificInfo(@PathVariable("id") Integer id,
			@RequestBody ProcessesSpecificInformation body) {
		return ResponseEntityTransformer.transform(generalCRUDService.update(processSpecificInfoRepository, id, body));
	}

	@Autowired
	private GetProcessSpecificInfoBySubProcessId getProcessSpecificInfoBySubProcessId;
	@Autowired
	private GeneralCRUDService generalCRUDService;
	@Autowired
	private ProcessSpecificInfoRepository processSpecificInfoRepository;
	@Autowired
	private ProcessSpecificInfoService processSpecificInfoService;
